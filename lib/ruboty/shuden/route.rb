    ...
    class Route
      ...
      def pretty
        ...
        @sections.each do |section|
          text += "%s(%s) -- " % [section.origin.name, section.origin.platform]
          text += "%s[%s(%s)] -> " % [section.way.line_name, section.way.time, section.way.duration]
        end
        last = @sections.last
        if last then
          text += "%s(%s)" % [last.dest.name, last.dest.platform]
        end
        return text
      end
